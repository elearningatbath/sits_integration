...
sits_block.get_acyear_options = function () {
	...
	html += '<option value="2009/10">2009/0</option>';
	html += '<option value="2010/1">2010/1</option>';
	html += '<option value="2011/2">2011/2</option>';
	html += '<option value="2012/3" selected="selected">2012/3</option>';
...
sits_block.appendNewMappingFormToPage = function (courseid) {
	...
	mapFormDiv.style.cssText = 'background-color: #9AB27C;';
	mapFormDiv.style.height = "20px";
	//mapFormDiv.innerHTML = html;
	mapFormDiv.id = 'id_' + courseid + '_new_map';
	//mapDiv.className = id;
	
	html = '<p class="new_map_' + courseid + '">';
	
	html += '<strong class="mpg_category">NEW MAPPING</strong>';

	html += '<label>Cohort Type</label> <select id="select_moduletype" name="moduletype" onchange="sits_block.switchModuleType()">';
	html += '<option value="unit" selected="selected">Unit</option>';
	html += '<option value="programme">Programme</option>';
	html += '</select>';
	html += '<label for="samis-module">SAMIS Code</label> <input style="width: 23em;" id="samis-module" type="text" value="" size="32" />';
	...
	YAHOO.util.Dom.get('id_' + courseid + '_new_mappings').appendChild(mapFormDiv);
};
...
sits_block.asyncRequest = function (op, xml) {	
	...
	message, getMapXML, srcXmlDoc, transaction, gid, courses, delElement;
...
sits_block.add_module_to_mappings = function () {
	...
	titleElement = YAHOO.util.Dom.get('which-samis-module'),
	course_id = sits_block.getTextContent(titleElement),
	codeElement = YAHOO.util.Dom.get('samis-module'),
	sits_code = codeElement.value.toUpperCase(),
	type = YAHOO.util.Dom.get('select_moduletype'),
	dayElement = YAHOO.util.Dom.get('id_add_map_day'),
	monElement = YAHOO.util.Dom.get('id_add_map_month'),
	yearElement = YAHOO.util.Dom.get('id_add_map_year'),
	acYearElement = YAHOO.util.Dom.get('mod-ac-year'),
	unenrolElement = YAHOO.util.Dom.get('id_add_map_unenrol_type'),
...
sits_block.validate_period_batch_xml = function (xmlString) {
	var i, checkId, id, n, start, end, code, acyear, codeCheck, acyearCheck, periodTable, periodTableRows, row, codeCheckElement, acyearCheckElement,
